...
/**
 * Compare our algorithm against the actual Minecraft classes. If these differ, our RNG seeding is
 * wrong.
 */
public class TestAgainstMinecraft {
  public static void main(String[] args) {
    ...
    System.out.println();
    System.out.printf(
        "%-12s %-20s %-20s %-8s%n", "Region", "Ours (chunkX,Z)", "MC (chunkX,Z)", "Match?");
    ...
    for (int rx = -2; rx <= 2; rx++) {
      for (int rz = -2; rz <= 2; rz++) {
        // --- Our algorithm ---
        long regionSeed =
            (long) rx * 341873128712L + (long) rz * 132897987541L + seed + (long) salt;
        ...
        System.out.printf(
            "(%2d, %2d)     (%4d, %4d)          (%4d, %4d)          %s%n",
            rx, rz, ourChunkX, ourChunkZ, mcChunkX, mcChunkZ, match ? "YES" : "NO <<<");
      }
    }
  }
}
