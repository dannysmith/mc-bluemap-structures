...
/**
 * Standalone test — prints structure positions for a known seed.
 * Compare output against Chunkbase to verify the algorithm.
 *
 * Run: ./gradlew run
 */
    ...
    public static void main(String[] args) {
        ...
        for (StructureType type : testTypes) {
            ...
            positions.sort(Comparator.comparingDouble(p ->
                    Math.sqrt((double) p.blockX() * p.blockX() + (double) p.blockZ() * p.blockZ())));
